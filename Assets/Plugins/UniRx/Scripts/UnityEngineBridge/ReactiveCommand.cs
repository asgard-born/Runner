...
namespace UniRx
{
    public interface IReadOnlyReactiveCommand<T> : IObservable<T>
    {
    }
    ...
    /// <summary>
    /// Represents input and output IReadOnlyReactiveCommand&lt;TIn, TOut&gt;
    /// </summary>
    public interface IReadOnlyReactiveCommand<out TIn, in TOut>
    {
        IDisposable Subscribe(Func<TIn, TOut> observerFunc);
    }
    
    /// <summary>
    /// Represents input and output ReactiveCommand&lt;TIn, TOut&gt;
    /// </summary>
    public interface IReactiveCommand<TIn, TOut> : IReadOnlyReactiveCommand<TIn, TOut>
    {
        IReadOnlyReactiveProperty<bool> CanExecute { get; }
    ...
    /// <summary>
    /// [Add] Reactive command with input and output, only last subscription will work, other will be ignored
    /// If Dispose called on Subscription, result will return default value of ouput type 
    /// </summary>
    public class ReactiveCommand<TIn, TOut> : IReactiveCommand<TIn, TOut>, IDisposable
    {
        private class DisposableTriggerFunction<TIn, TOut> : IDisposable
        {
            private Func<TIn, TOut> _triggerFunc;
            ...
            public IDisposable SetTriggerFunction(Func<TIn, TOut> triggerFunc)
            {
                _triggerFunc = triggerFunc;
                return this;
            }

            public Func<TIn, TOut> GetTriggerFunction()
            {
                return _triggerFunc;
            }
        }
        ...
        readonly DisposableTriggerFunction<TIn, TOut> disposableTriggerFunc;
        readonly IDisposable canExecuteSubscription;
        ...
        public ReactiveCommand()
        {
            this.disposableTriggerFunc = new DisposableTriggerFunction<TIn, TOut>();
        ...
        {
            if (!canExecute.Value) return default;
            ...
            Func<TIn, TOut> triggerFunc = disposableTriggerFunc.GetTriggerFunction();
            return triggerFunc == null ? default : triggerFunc.Invoke(parameter);
        }

        /// <summary>Force push parameter to subscribers.</summary>
        public TOut ForceExecute(TIn parameter)
        {
            if (IsDisposed)
            {
                throw new ObjectDisposedException("");
        ...
        /// <summary>Subscribe execute.</summary>
        public IDisposable Subscribe(Func<TIn, TOut> observerFunc)
        {
            if (IsDisposed)
            {
                throw new ObjectDisposedException("");
            ...
            return disposableTriggerFunc.SetTriggerFunction(observerFunc);
        }
