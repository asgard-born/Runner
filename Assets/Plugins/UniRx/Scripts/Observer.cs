...
namespace UniRx
{
  public static class Observer
  {
    ...
    internal static IObserver<T> CreateSubscribeWithState2Observer<T, TState1, TState2>(TState1 state1, TState2 state2,
      Action<T, TState1, TState2> onNext, Action<Exception, TState1, TState2> onError,
    ...
    class Subscribe<T, TState1, TState2, TState3> : IObserver<T>
    {
      ...
      public Subscribe(TState1 state1, TState2 state2, TState3 state3, Action<T, TState1, TState2, TState3> onNext,
        Action<Exception, TState1, TState2, TState3> onError, Action<TState1, TState2, TState3> onCompleted)
      {
        this.state1 = state1;
        this.state2 = state2;
  ...
  {
    public class ObservatorDisposable : IDisposable
    {
      public static Dictionary<Type, int> TotalSubs
        => _count;

      private static readonly Dictionary<Type, int> _count = new Dictionary<Type, int>();
      private readonly IDisposable _realDisposable;
      private readonly Type _type;
#if SUBSCRIPTIONS_DEBUG
      public static Dictionary<string, int> _debugStacksCount = new Dictionary<string, int>();
      public static Dictionary<string, int> _debugStacksSnapshot = new Dictionary<string, int>();
      private string _stackTrace;
      public static void DebugTakeSnapshot()
      {
        _debugStacksSnapshot = _debugStacksCount.ToDictionary(
          entry => entry.Key, 
          entry => entry.Value);
      }

      public static Dictionary<string, int> DebugGetSnapshotDiff()
      {
        Dictionary<string, int> diff = new Dictionary<string, int>();
        foreach (KeyValuePair<string, int> pair in _debugStacksCount)
        {
          if (_debugStacksSnapshot.ContainsKey(pair.Key))
          {
            if (_debugStacksSnapshot[pair.Key] == _debugStacksCount[pair.Key])
            {
              continue;
            }
          }
...
#endif
      public ObservatorDisposable(IDisposable disposable, Type type)
      {
#if SUBSCRIPTIONS_DEBUG
        _stackTrace = Environment.StackTrace;
        if(!_debugStacksCount.ContainsKey(_stackTrace))
          _debugStacksCount.Add(_stackTrace,0);
        _debugStacksCount[_stackTrace]++;
#endif
        _realDisposable = disposable;
        _type = type;
      ...
      public void Dispose()
      {
        _realDisposable?.Dispose();
        
#if SUBSCRIPTIONS_DEBUG
        if (_debugStacksCount.ContainsKey(_stackTrace))
        {
          _debugStacksCount[_stackTrace]--;
          if (_debugStacksCount[_stackTrace] == 0)
            _debugStacksCount.Remove(_stackTrace);
        }
#endif
        if (_count.ContainsKey(_type))
        {
          _count[_type]--;
          if (_count[_type] <= 0)
            _count.Remove(_type);
    ...
    {
      return new ObservatorDisposable(source.Subscribe(UniRx.InternalUtil.ThrowObserver<T>.Instance), typeof(T));
    }

    public static IDisposable Subscribe<T>(this IObservable<T> source, Action<T> onNext)
    {
      return new ObservatorDisposable(
        source.Subscribe(Observer.CreateSubscribeObserver(onNext, Stubs.Throw, Stubs.Nop)), typeof(T));
    }
